...
const MatchesPage: React.FC<MatchesPageProps> = ({
  ...
  const { topMatches, otherMatches } = useMemo(() => {
    ...
    const matchesWithScores = filteredUsers.map((user) => {
      const teachesLearnable = user.teaches.filter((skill) =>
        learnSkillIds.has(skill.id),
      );
      const learnsTeachable = user.learns.some((skill) =>
        currentUser.teaches.some((s) => s.id === skill.id),
      );
    ...
    const directMatches = sortedMatches.filter(
      (m) => m.matchingSkills.length > 0,
    );
    const nonDirectMatches = sortedMatches.filter(
      (m) => m.matchingSkills.length === 0,
    );
  ...
  const getButtonState = (user: User, isHero: boolean = false) => {
    ...
    const baseClasses =
      "w-full flex items-center justify-center gap-2 font-bold transition-all active:scale-95";

    // Hero buttons are larger/more prominent
    const sizeClasses = isHero
      ? "py-3 px-8 text-base rounded-xl shadow-xl"
      : "py-2.5 px-4 text-sm rounded-xl shadow-lg";
      ...
      return (
        <div className="flex gap-2">
          <button
            onClick={(e) =>
              handleActionClick(e, () =>
                handleRequest(incomingRequest.id, "accepted"),
              )
            }
            className={`flex-1 ${sizeClasses} bg-emerald-500 text-white hover:bg-emerald-400 shadow-emerald-500/20`}
          >
            Accept
          </button>
          <button
            onClick={(e) =>
              handleActionClick(e, () =>
                handleRequest(incomingRequest.id, "declined"),
              )
            }
            className={`flex-1 ${sizeClasses} bg-slate-700 text-slate-300 hover:bg-slate-600`}
          >
            Decline
          </button>
        </div>
    ...
    if (hasPendingRequest) {
      return (
        <button
          disabled
          className={`${baseClasses} ${sizeClasses} bg-slate-800 text-slate-500 border border-slate-700 cursor-not-allowed`}
        >
          Request Sent
        </button>
      );
