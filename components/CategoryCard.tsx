...
const CategoryCard: React.FC<CategoryCardProps> = ({ category, onClick }) => {
    ...
    const darkColorClasses: Record<string, string> = {
        ...
        purple: 'dark:from-purple-500/30 dark:to-slate-900/10 dark:border-purple-500/50 dark:text-purple-300',
        emerald: 'dark:from-emerald-500/30 dark:to-slate-900/10 dark:border-emerald-500/50 dark:text-emerald-300',
        rose: 'dark:from-rose-500/30 dark:to-slate-900/10 dark:border-rose-500/50 dark:text-rose-300',
        slate: 'dark:from-slate-500/30 dark:to-slate-900/10 dark:border-slate-500/50 dark:text-slate-300'
    };
    ...
    const hoverClass = 'hover:border-slate-400/50 dark:hover:border-white/50';

    return (
        <button 
            onClick={() => onClick?.(category.id)}
            className={`group relative p-6 rounded-2xl bg-gradient-to-br border overflow-hidden transition-all duration-300 text-left ${cardClass} ${hoverClass}`}
        >
