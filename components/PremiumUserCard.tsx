  ...
  return (
    <motion.div
      ...
      className={`relative group rounded-3xl overflow-hidden cursor-pointer transition-all duration-300 ${
        isTopMatch
          ? "bg-surface-highlight border border-sky-500/20 shadow-xl shadow-sky-500/10 hover:shadow-sky-500/20"
          : "bg-surface border border-border hover:border-sky-500/30 hover:shadow-lg"
      }`}
    ...
    >
      ...
      {isTopMatch && (
        <div className="absolute top-0 right-0 p-[1px] rounded-bl-2xl bg-gradient-to-br from-sky-500 to-purple-500">
          <div className="bg-surface rounded-bl-2xl px-3 py-1">
        ...
        <div className="flex items-start gap-4 mb-4">
          <div className="relative">
            <img
              src={user.avatarUrl}
              alt={user.name}
              className={`rounded-full object-cover shadow-lg ${isTopMatch ? "w-16 h-16 ring-2 ring-purple-500/50" : "w-14 h-14 border border-border"}`}
            />
            {user.isOnline && (
              <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-emerald-500 border-2 border-surface rounded-full shadow-sm"></span>
            )}
          </div>
