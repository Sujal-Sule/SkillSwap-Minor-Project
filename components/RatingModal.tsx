...
interface RatingModalProps {
    ...
    session: Session;
    targetUser: User;
    onSubmit: (sessionId: string, stars: number, feedback: string) => any;
    title?: string;
    placeholder?: string;
}
...
const StarIcon: React.FC<{ filled: boolean, onClick: () => void, onMouseEnter: () => void, onMouseLeave: () => void }> = ({ filled, onClick, onMouseEnter, onMouseLeave }) => (
    <button type="button" onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
        <svg className={`w-8 h-8 cursor-pointer transition-colors ${filled ? 'text-amber-400' : 'text-slate-400 dark:text-slate-500 hover:text-amber-300'}`} fill={filled ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.5" viewBox="0 0 20 20">
...
const RatingModal: React.FC<RatingModalProps> = ({ isOpen, onClose, session, targetUser, onSubmit, title, placeholder }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (stars > 0 && !submitting) {
            setSubmitting(true);
            try {
                await onSubmit(session.id, stars, feedback);
            } catch (err) {
                console.error(err);
            } finally {
                setSubmitting(false);
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end space-x-3">
                    <button
                        ...
                        onClick={onClose}
                        disabled={submitting}
                        className="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-200 font-semibold rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 disabled:opacity-50 transition-colors"
                    >
                        Cancel
                    </button>
                    <button
